...
namespace Internal.Cryptography.Pal
{
    internal sealed partial class CertificatePal : IDisposable, ICertificatePal
    {
        ...
        public unsafe void SetCspPrivateKey(AsymmetricAlgorithm key)
        {
            ...
            CspKeyContainerInfo keyContanerInfo;
            switch (key.SignatureAlgorithm)
            {
                case GostConstants.XmlSignatureAlgorithm2001:
                {
                    ...
                    keyContanerInfo = asymmetricAlgorithm.CspKeyContainerInfo;
                    break;
                }
            ...
            fixed (char* keyContainerName = keyContanerInfo.KeyContainerName)
            fixed (char* providerName = keyContanerInfo.ProviderName)
            {
                ...
                keyProvInfo.dwProvType = keyContanerInfo.ProviderType;
                keyProvInfo.dwFlags = keyContanerInfo.MachineKeyStore 
                    ? CryptAcquireContextFlags.CRYPT_MACHINE_KEYSET 
                    : CryptAcquireContextFlags.None;
                keyProvInfo.cProvParam = 0;
                keyProvInfo.rgProvParam = IntPtr.Zero;
                keyProvInfo.dwKeySpec = (int)keyContanerInfo.KeyNumber;
