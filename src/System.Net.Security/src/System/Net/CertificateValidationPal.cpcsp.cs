            ...
            string hostName)
        {
            ...
            if (checkCertName)
            {
                unsafe
                {
                    ...
                    //convert 2 byte wchar to 4 byte wchar
                    var buf  = new byte[(hostName.Length + 1) * 4];
                    var byte_str = Encoding.Unicode.GetBytes(hostName);
                    for (int i = 0; i < hostName.Length; i++)
                    {
                        buf[i*4] = byte_str[i*2];
                        buf[i*4 + 1] = byte_str[i*2 + 1];
                    }
                    fixed (byte* namePtr = buf)
                    {
                        eppStruct.pwszServerName = (char*)namePtr;
                        cppStruct.dwFlags |=
